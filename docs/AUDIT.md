...
| `prepare_data.py` | Canonical TSV/ZIP handling and duplicate-copy protection work. | Reuse unchanged. |
...
New primary workflow: strict ingest → NFKC/mark-preserving normalization → optional AnyAscii fields → sharded character TF-IDF + SQLite BM25 → union with channel scores/ranks → fixed-order features → LightGBM and SGD reference → calibration-only threshold → untouched holdout → artifacts → fresh test indexes → streamed/resumable inference → strict validation → package.
